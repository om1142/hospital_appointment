    ...
    try {
        ...
        if (customer.isVerified) {
            ...
            if (customer && auth) {
                ...
                res.status(201).render('customer/edit', { customer, err: 'You have logged in successfully.' });
            } else {
...
const manager_edit_patch = async (req, res) => {
    try {
        ...
        const now = new Date();
        const age = now.getFullYear() - manager.date.getFullYear();

        // Validate birthdate constraints
        if (age < 5) {
            return res.status(500).render('manager/edit', { manager, err: 'You must be at least 5 years old to sign up.' });
        } else if (age > 110) {
...
const cadet_edit_patch = async (req, res) => {
    try {
        ...
        if (cadet) {
            ...
            const now = new Date();
            const age = now.getFullYear() - cadet.date.getFullYear();

            // Validate birthdate constraints
            if (age < 5) {
                return res.status(500).render('cadet/edit', { customer, err: 'You must be at least 5 years old to sign up.' });
            } else if (age > 110) {
